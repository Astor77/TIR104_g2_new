...
def sele_movie_data_flow() -> None:
    logger = get_run_logger()
    try:
        logger.info("f4_sele_movie_data_flow() 開始運行...")
        dfTWMovie = e_tw_read_csv()
        MovieIds = dfTWMovie["MovieId"].tolist()
        # 等下載完成
        logger.info("等待抓取json完成...")
        sale_future = e_get_tw_one_movie_sale.submit(MovieIds)
        # 並行抓上映日期
        logger.info("等待抓取日期完成...")
        release_future = e_get_tw_one_movie_release_date.submit(MovieIds)

        sale_future.result()
        logger.info("✅ 完成抓取單片json")
        release_date_list = release_future.result()
        logger.info("✅ 完成抓取單片上映日期")
