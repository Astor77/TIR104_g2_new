...
def save_as_csv(dataframe: pd.DataFrame, dir_path: str | Path, file_name: str) -> None:
    """
    將 DataFrame 儲存為 CSV 檔案，並且保留原始dataframe的資料型態
    Args:
...
def save_as_json(data: list | dict, dir_path: str | Path, file_name: str) -> None:
    """
    將 list 或 dict 儲存為 JSON 檔案
    Args:
        data (list| dict): 要儲存的原始資料
        dir_path (str): 儲存的資料夾完整路徑
        file_name (str): 儲存的檔案名稱字串
    """
    try:
